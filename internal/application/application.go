...
func (a *App) encodeCursorFiltered(id string, next uint64, prev string, f AuditFilter) string {
	generation := a.Store.Generation()
	x := auditCursor{CaseID: id, NextRevision: next, PreviousDigest: prev, StoreGeneration: generation, Signature: a.cursorSignature(id, next, prev, generation) + ":" + a.cursorFilterKey(f)}
	b, _ := json.Marshal(x)
...
func (a *App) decodeCursorFiltered(raw, id string, f AuditFilter) (auditCursor, error) {
	...
	want := a.cursorSignature(id, x.NextRevision, x.PreviousDigest, x.StoreGeneration) + ":" + a.cursorFilterKey(f)
	if subtle.ConstantTimeCompare([]byte(want), []byte(x.Signature)) != 1 {
		return x, domain.ErrInvalidCursor
	}
	if x.StoreGeneration != a.Store.Generation() {
		return x, domain.ErrInvalidCursor
	}
	return x, nil
}
...
func (a *App) cursorSignature(caseID string, next uint64, prev string, generation uint64) string {
	h := sha256.New()
	h.Write(a.cursorKey[:])
	fmt.Fprintf(h, "%s\x00%d\x00%s\x00%d", caseID, next, prev, generation)
	return hex.EncodeToString(h.Sum(nil))
}
func (a *App) encodeCursor(caseID string, next uint64, prev string) string {
	generation := a.Store.Generation()
	x := auditCursor{CaseID: caseID, NextRevision: next, PreviousDigest: prev, StoreGeneration: generation}
	x.Signature = a.cursorSignature(caseID, next, prev, generation)
	b, _ := json.Marshal(x)
	return base64.RawURLEncoding.EncodeToString(b)
}
func (a *App) decodeCursor(raw, caseID string) (auditCursor, error) {
	...
	want := a.cursorSignature(x.CaseID, x.NextRevision, x.PreviousDigest, x.StoreGeneration)
	if subtle.ConstantTimeCompare([]byte(want), []byte(x.Signature)) != 1 {
		return x, domain.ErrInvalidCursor
	}
	if x.StoreGeneration != a.Store.Generation() {
