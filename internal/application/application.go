...
// ArchiveContext 允许传输层在请求取消时停止等待。
func (a *App) ArchiveContext(ctx context.Context, id string, expected uint64, req, actor string) (*domain.ProvenanceCase, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	type result struct {
		caseValue *domain.ProvenanceCase
		err       error
	}
	done := make(chan result, 1)
	go func() {
		caseValue, err := a.Archive(id, expected, req, actor)
		done <- result{caseValue: caseValue, err: err}
	}()
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	case outcome := <-done:
		return outcome.caseValue, outcome.err
	}
}
