...
type App struct {
	Store          *store.Store
	cursorKey      [32]byte
	preflightCache *archivePreflightCache
}

type archivePreflightCache struct {
	caseID   string
	revision uint64
	report   *domain.ArchivePreflight
}
...
func (a *App) ArchivePreflight(id string) (*domain.ArchivePreflight, error) {
	...
	if cached := a.preflightCache; cached != nil && cached.caseID == id && cached.revision == c.Revision {
	...
	a.preflightCache = &archivePreflightCache{caseID: id, revision: c.Revision, report: r}
	return r, nil
}
