...
type App struct {
	...
	caseListCache map[string]CaseListPage
}
...
func (a *App) ListCases(f CaseListFilter, cursor string, size int) (CaseListPage,error) {
	...
	cacheKey := fp(struct { Filter CaseListFilter `json:"filter"`; Cursor string `json:"cursor"`; Size int `json:"size"` }{f, cursor, size})
	a.caseListMu.Lock()
	if cached, ok := a.caseListCache[cacheKey]; ok { a.caseListMu.Unlock(); return cloneCaseListPage(cached), nil }
	a.caseListMu.Unlock()
	...
	end:=start+size; if end>len(filtered){end=len(filtered)}; p:=CaseListPage{Cases:filtered[start:end],StatusCounts:counts,TotalCount:len(filtered)}; if end<len(filtered){ k:=caseListKey(filtered[end-1]); raw,_:=json.Marshal(struct{Key string `json:"key"`; Sig string `json:"sig"`}{k,caseFilterKey(f)+":"+k}); p.NextCursor=base64.RawURLEncoding.EncodeToString(raw) }
	a.caseListMu.Lock(); a.caseListCache[cacheKey] = cloneCaseListPage(p); a.caseListMu.Unlock()
	return p,nil
}
func cloneCaseListPage(p CaseListPage) CaseListPage { b,_:=json.Marshal(p); var out CaseListPage; _=json.Unmarshal(b,&out); return out }
...
func New(s *store.Store) *App { a := &App{Store: s, caseListCache: map[string]CaseListPage{}}; _, _ = rand.Read(a.cursorKey[:]); return a }
