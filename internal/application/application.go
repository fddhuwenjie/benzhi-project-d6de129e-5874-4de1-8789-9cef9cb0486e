...
type App struct {
	Store        *store.Store
	cursorKey    [32]byte
	cursorHasher hash.Hash
	cursorText   bytes.Buffer
}
...
func New(s *store.Store) *App { a := &App{Store: s, cursorHasher: sha256.New()}; _, _ = rand.Read(a.cursorKey[:]); return a }
...
func (a *App) cursorFilterKey(f AuditFilter) string {
	b, _ := json.Marshal(f)
	a.cursorHasher.Reset()
	a.cursorText.Reset()
	_, _ = a.cursorText.Write(b)
	_, _ = a.cursorHasher.Write(a.cursorText.Bytes())
	return hex.EncodeToString(a.cursorHasher.Sum(nil))
}
...
func (a *App) cursorSignature(caseID string, next uint64, prev string) string {
	a.cursorHasher.Reset()
	a.cursorText.Reset()
	_, _ = a.cursorText.Write(a.cursorKey[:])
	_, _ = fmt.Fprintf(&a.cursorText, "%s\x00%d\x00%s", caseID, next, prev)
	_, _ = a.cursorHasher.Write(a.cursorText.Bytes())
	return hex.EncodeToString(a.cursorHasher.Sum(nil))
}
